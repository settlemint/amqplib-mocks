...
function findHandlers( connection, exchange, routingKey ) {
	...
	const filtered = _.filter( exchange.bindings, binding =>
		binding.regex.test( routingKey )
	);
	return _.transform(
		filtered,
		( result, binding ) => {
			...
			if ( binding.exchangeName ) {
				const boundExchange =
					connection.exchanges[ binding.exchangeName ];
				const consumers = findHandlers(
					connection,
					boundExchange,
					routingKey
				);
...
class Channel {
	constructor( connection ) {
		...
		this.checkExchange = sinon.stub().resolves();

		this.trackedMessages = [];

		this.assertQueue = sinon.stub().callsFake( async ( queue, opt ) => {
			setIfUndefined( this.connection.queues, queue, {
				messages: [],
				consumers: {},
				options: opt
			} );
		...
		this.assertExchange = sinon.stub().callsFake( async ( exchange, opt ) => {
			setIfUndefined( this.connection.exchanges, exchange, {
				bindings: [],
				options: opt
			} );
			return { exchange };
		...
		this.bindExchange = sinon
			.stub()
			.callsFake( async ( destination, source, pattern, args ) => {
				...
				this.connection.exchanges[ source ].bindings.push( {
					regex,
		...
		this.bindQueue = sinon
			.stub()
			.callsFake( async ( queue, exchange, pattern, args ) => {
				if ( !this.connection.exchanges[ exchange ] ) {
					throw new Error(
						`Bind to non-existing exchange: ${ exchange }`
					);
				}
				const regex = generateBindingRegex( pattern );
				this.connection.exchanges[ exchange ].bindings.push( {
					regex,
					queueName: queue
				} );
				return {};
		...
		this.consume = sinon.stub().callsFake( async ( queueName, handler ) => {
			...
			if ( !queue ) {
				throw new Error(
					`Consuming from non-existing queue: ${ queueName }`
				);
			}
		...
		this.publish = sinon
			.stub()
			.callsFake(
				async ( exchangeName, routingKey, content, properties ) => {
					const exchange = this.connection.exchanges[ exchangeName ];
					if ( !exchange ) {
						throw new Error(
							`Publish to non-existing exchange: ${ exchangeName }`
						);
					}
					const consumers = findHandlers(
						this.connection,
					...
					const message = {
						fields: { routingKey, exchange: exchangeName },
						content,
						properties
					};
		...
		this.sendToQueue = sinon
			.stub()
			.callsFake( async ( queueName, content, properties ) => {
				const queue = this.connection.queues[ queueName ];
				if ( !queue ) {
					return true;
				}
